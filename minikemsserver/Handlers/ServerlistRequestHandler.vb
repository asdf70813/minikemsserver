...
#End Region
    Sub New(ByVal packetReader As PacketReader, ByVal c As MapleClient)
        ...
        For Each world As MapleWorld In Server.Worlds
            packet = MaplePacketHandler.GetServerList(world)
            c.SendPacket(packet)
        Next
        packet = MaplePacketHandler.EndOfGetServerList
        c.SendPacket(packet)
        packet = MaplePacketHandler.SelectWorld
        c.SendPacket(packet)
        packet = MaplePacketHandler.sendRecommended(Server.Worlds(0))
        c.SendPacket(packet)
        Me.Dispose()
    End Sub
End Class
